package engeto.DU.PodmínkyCyklySwitch;
...
public class Main {
    ...
    public static void readIntsFromInputAndPrintItUntilNegative() {
        ...
        while (input >= 0) {
            ...
            if (input >= 0) System.out.println("You entered: " + input);
            else System.out.println("You eneterd negative number, proces was terminated");
        }
    }
    ...
    public static int sumAllInputUntilNegative() {
        ...
        int total=0;
         while (input >= 0){
            input = Support.safeReadInt();
            System.out.println("You entered: " + input);
            if (input >= 0) System.out.println("Sum of all positiv numbers = " + (total+=input));
            else System.out.println("You eneterd negative number,Sum of all positiv numbers = " + total);
        }
        return total;
    }
    ...
    public static List storeAllInputInArrayListUntilNegative() {
        ArrayList<Integer>listOfPositivInputs = new ArrayList<>();
        int input = 0;
        while (input >= 0) {
            input = Support.safeReadInt();
            if (input >= 0) listOfPositivInputs.add(input);
            else System.out.println("You eneterd negative number, process is terminated.");
        }
        return listOfPositivInputs;
    }
    ...
    public static void printIntegersWithReplace(List<Integer> list) {
        String resultConvertToText;
        for (int a : list) {
            resultConvertToText = String.valueOf(a);
            if (resultConvertToText.equals("1")) resultConvertToText = "one";
            if (resultConvertToText.equals("2")) resultConvertToText = "two";
            if (resultConvertToText.equals("3")) resultConvertToText = "three";
            System.out.println(resultConvertToText);
        }
    }
    //endregion
